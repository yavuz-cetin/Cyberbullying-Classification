...
Original file is located at
    https://colab.research.google.com/drive/1H64Ht8nM3wIoWpKUBduLuFtwd2pMBToV
"""
...
fasttext_train_data = [text.split() for text in X_train]

import gensim
from gensim.models import FastText

model = FastText(sentences=fasttext_train_data, vector_size=100, window=5, min_count=1, workers=4)
...
for word, i in tokenizer.word_index.items():
    if word.strip() in ft_model.wv:
        embedding_matrix[i] = ft_model.wv[word.strip()]
    else:
...
checkpoint = ModelCheckpoint("best_model_fasttext_lstm.keras", monitor='val_accuracy', verbose=1, save_best_only=True, mode='max')

model.fit(X_train_pad, y_train, epochs=30, batch_size=64, validation_data=(X_test_pad, y_test), callbacks=[checkpoint])

model = load_model("best_model_fasttext_lstm.keras")
